...
def run_anti_analysis(excitation_obj, emission_obj, excitation_name: str, emission_name: str, config: dict) -> dict:
    ...
    excitation_norm, excitation_summary, excitation_warnings = normalize_max_per_well_with_summary(excitation_df[["Wavelength"] + wells], wells, "Excitation")
    emission_norm, emission_summary, emission_warnings = normalize_max_per_well_with_summary(emission_df[["Wavelength"] + wells], wells, "Emission")
    ...
    outputs.extend([str(excitation_out), str(emission_out), str(summary_path)])

    grid_path = run_dir / "figures" / "anti_grid_plots.pdf"
    plot_grid_two_series(excitation_norm, emission_norm, wells, ("Excitation", "Emission"), grid_path, config, "anti normalized spectra", normalized=True, module_key="anti")
    outputs.append(str(grid_path))

    report_path = run_dir / "report" / "anti_combined_report.pdf"
    plot_grid_two_series(excitation_norm, emission_norm, wells, ("Excitation", "Emission"), report_path, config, "anti normalized spectra report", normalized=True, module_key="anti")
    ...
    result.update({"excitation_normalized": excitation_norm, "emission_normalized": emission_norm, "summary": summary, "output_files": result["metadata"]["output_files"]})
    return result
